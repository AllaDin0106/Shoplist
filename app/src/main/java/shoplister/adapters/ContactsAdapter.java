...
public class ContactsAdapter extends BaseAdapter<Contact, ContactsAdapter.ViewHolder>
{
    // Requirement 2: Cookbook Listener #1 - Member Class
    class DeleteContact implements View.OnLongClickListener
    {
        Contact contact;

        public DeleteContact(Contact contact) { this.contact = contact; }

        @Override
        public boolean onLongClick(View v)
        {
            // Requirement 3: DialogFragment - Confirm contact deletion
            new DeleteContactDialog(contact).show();
            return true;
        }
    }
    ...
    void initAdapter()
    {
        ...
        adapter = new FirebaseRecyclerAdapter<Contact, ViewHolder>(options)
        {
            ...
            protected void onBindViewHolder(@NonNull ViewHolder holder, int position, @NonNull Contact model)
            {
                ...
                holder.contact_container.setOnLongClickListener(new DeleteContact(model));
